...
import { jobCategoryKey, provinceKey } from "@/cache-key";
import locationService from "@/services/location.service";
import { JobCategory, Provice } from "@/types";
...
export function HeroSearch() {
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    router.push(
      `/discover/jobs-by-field${params.size ? `?${params.toString()}` : ""}`,
    );
  }
  ...
  return (
    <section className="relative overflow-hidden bg-gradient-to-br from-emerald-950 via-emerald-800 to-emerald-600">
      ...
      <div
        aria-hidden
        className="absolute -left-20 top-10 size-72 rounded-full bg-emerald-300/10 blur-3xl"
      />
      <div
        aria-hidden
        className="absolute -right-20 bottom-0 size-80 rounded-full bg-cyan-300/10 blur-3xl"
      />
      <div className="relative mx-auto max-w-7xl px-4 py-12 sm:px-6 md:py-16 lg:py-20">
        <div className="grid items-center gap-8 lg:grid-cols-[1fr_.92fr] lg:gap-12">
          <div className="text-center lg:text-left">
            <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-4 py-2 text-xs font-semibold text-emerald-50 backdrop-blur-sm">
              <BriefcaseBusiness className="size-3.5" /> Hơn 10.000 cơ hội mới
            ...
            <p className="mx-auto mt-4 max-w-2xl text-pretty text-sm leading-6 text-emerald-50/85 md:text-base lg:mx-0">
              Khám phá cơ hội từ doanh nghiệp đã xác thực, theo đúng chuyên môn,
              địa điểm và định hướng nghề nghiệp của bạn.
            </p>
          </div>
          <div className="relative mx-auto w-full max-w-xl">
            <div className="absolute -inset-3 rounded-[2rem] bg-white/10 blur-xl" />
            <div className="relative overflow-hidden rounded-[1.75rem] border border-white/25 bg-white shadow-2xl shadow-emerald-950/25">
              <Image
                src="/images/banner-home.webp"
                alt="Ứng viên khám phá cơ hội nghề nghiệp trên TopCV"
                width={1536}
                height={1024}
                priority
            ...
            <span className="absolute -bottom-4 left-5 rounded-2xl border border-white/30 bg-white/95 px-4 py-3 text-xs font-bold text-emerald-800 shadow-xl backdrop-blur">
              10.000+ cơ hội mỗi ngày
            </span>
          </div>
        </div>
        ...
        <form
          onSubmit={submit}
          className="mx-auto mt-8 max-w-6xl rounded-2xl border border-white/25 bg-white p-2.5 shadow-2xl shadow-emerald-950/25 md:rounded-3xl md:p-3"
        >
          <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-[minmax(0,1fr)_260px_220px_auto] md:items-center">
            ...
            <div className="flex h-13 min-w-0 items-center gap-2 rounded-xl bg-slate-50 px-2">
              ...
              >
                <SelectTrigger
                  ...
                  className="h-11 min-w-0 flex-1 border-0 bg-transparent shadow-none focus-visible:ring-0 [&_[data-slot=select-value]]:min-w-0 [&_[data-slot=select-value]]:block [&_[data-slot=select-value]]:truncate"
                >
                  <SelectValue>
                    {category === "all" ? "Tất cả danh mục nghề" : category}
                  </SelectValue>
                </SelectTrigger>
                <SelectContent
                ...
                >
                  ...
                  {categoriesPending && (
                    <p className="px-3 py-2 text-sm text-slate-500">
                      Đang tải danh mục nghề...
                    </p>
                  )}
                  {categoriesError && (
                    <div className="px-3 py-2 text-sm text-red-600">
                      Không thể tải danh mục nghề.{" "}
                      <button
                        type="button"
                        className="underline"
                  ...
                  {categories?.data.map((category: JobCategory) => (
            ...
            <div className="flex h-13 min-w-0 items-center gap-2 rounded-xl bg-slate-50 px-2">
              ...
              >
                ...
                >
                  ...
                  {provinces?.data.map((province: Provice) => (
            ...
            <Button
              size="lg"
              type="submit"
              className="h-13 gap-2 rounded-xl bg-emerald-600 px-7 text-sm font-semibold shadow-lg shadow-emerald-600/20 transition-all hover:-translate-y-0.5 hover:bg-emerald-700 hover:shadow-xl"
            >
        ...
        {/* <div className="mx-auto mt-5 flex max-w-6xl flex-wrap items-center justify-center gap-2">
          <span className="mr-1 text-sm text-emerald-50/80">
            Tìm kiếm phổ biến:
          </span>
          {QUICK_TAGS.map((tag) => (
            <button
        ...
        <div className="mx-auto mt-8 grid max-w-3xl grid-cols-1 gap-3 text-xs text-emerald-50/85 sm:grid-cols-3">
          <span className="flex items-center justify-center gap-2">
            <ShieldCheck className="size-4 text-emerald-300" /> Doanh nghiệp đã
            xác thực
          </span>
          <span className="flex items-center justify-center gap-2">
            <BriefcaseBusiness className="size-4 text-emerald-300" /> Tin tuyển
            dụng minh bạch
          </span>
          <span className="flex items-center justify-center gap-2">
            <ArrowRight className="size-4 text-emerald-300" /> Ứng tuyển nhanh
            với CV đã lưu
          </span>
        </div>
      </div>
